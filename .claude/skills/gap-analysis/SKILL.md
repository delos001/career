...
- **Step 4c - Walk `evidence?` items one at a time.** For each, surface the requirement text and the sub-agent's `missing` field; the user provides clarifying information. Judge critically in main skill — does the information fully close the gap, partially address it, or not close it? A full closure means the candidate can honestly claim the requirement on the CV without qualification. A partial match means genuine transferable experience exists but a real gap remains underneath. State the assessment and reasoning explicitly before assigning status. Do not accept context at face value; push back if coverage is thin. Outcomes:
  - Fully closes the gap → assign `closed`; queue a **closure** staging entry per [[respect-profile-doc-conventions]].
  - Partially addresses it (transferable but not a full match) → assign `partial-match`; note the transferable element and the gap that remains. When the user surfaced substantive experience the inventory under-represents, queue an **enrichment** staging entry for it; a partial match is not a closure and must never be staged as one.
  - Does not close it → re-categorize as `interview-deferred` or `unresolved`.
- A `covered` requirement can also surface under-represented experience during the walk. Queue that as an **enrichment** entry too; it changes no status and no score.
...
- **Step 6a - Append staging entries (assign PU-NNN).** For each queued staging entry, run `python scripts/staging_append.py --folder <app_folder> --kind <closure|enrichment> --content-file <path> --label <short-label>`, plus the requirement flags for its kind. The capture date, APP-NNN, company, role, and the five axis values are read from the session log; pass them explicitly only to override a derived value.
  - `--kind closure`: `--closed-requirement <CR-NNN> --requirement-text-short <text>`.
  - `--kind enrichment`: `--related-requirement "<CR-NNN> - <text>"`, or `--related-requirement "n/a (<short reason>)"` when no requirement anchors the capture.

  Script assigns the next `PU-NNN` and prints the assigned ID. Capture each closure's `PU-NNN` paired with the `CR-NNN` it closes; enrichments have no closure pointer and are NOT passed to `--appended-pu` at Phase 7. This runs before the artifact is assembled so the closure pointers exist when the document is rendered.
