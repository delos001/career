---
name: work-state-builder
description: Build or refresh a work-state value file in rules/work-states/. Researches the target work-state, drafts the value file (Identity, Achievement framing, Adjacency), reconciles against sibling files, runs QC with auto-fix, and updates the registry. Invoke when a work-state value is "File deferred" in the registry, or when an existing file needs a periodic refresh.
---

# work-state-builder - build or refresh a work-states axis value file
...
**Building or refreshing a work-state value file.**
...
**Resolving the target work-state value and mode.**
...
- Run `python scripts/axis_registry.py list work-states`. It returns
...
**Researching the target work-state.**

- Input: target value, siblings list (from Phase 1).
- Dispatch the `work-state-builder-research` subagent with the inputs it
  declares in `.claude/agents/work-state-builder-research.md`:
  - `axis`: `work-states`
...
- Output: research findings block per the agent's return schema (identity
  framing, achievement-framing signals, adjacency translation
...
- Draft the value file content per the per-axis schema: frontmatter
  (`work-state: <value>`, `last_researched: <current YYYY-MM>`),
  `Used by:` header, then Identity, Achievement framing, Adjacency.
- Match the voice and density of existing files in `rules/work-states/`.
  `greenfield.md` is the canonical reference.
- **Identity scoping discipline.** The Identity section names what the
  work-state IS, and the explicit conditions where the work-state does
  not fit (with sibling redirection). Sibling work-states' identities
  must not be paraphrased here (D1 enforces).
- **Create mode only**: also draft the one-line registry-entry bullet
  Phase 6 will splice into `rules/work-states/registry.md`. Format:
...
**Reconciling the draft against the rest of the rules/work-states/ folder.**
...
- **Create mode pre-step (extract sibling Adjacency slices).** For each
  ...
  ```
  python scripts/axis_registry.py slice work-states <sibling_value> \
    --section Adjacency
  ```

  Capture the stdout as `adjacency_text` for that sibling.
- Dispatch the `work-state-builder-reconciler` subagent with the inputs it
  declares in `.claude/agents/work-state-builder-reconciler.md`:
  - `axis`: `work-states`
  - `value`: the target value
...
- The QC runs in two halves per `rules/quality_control/qc-work-state-builder.md`:
  ...
  **Subagent half - judgment checks.** Dispatch `qc-work-state-builder` with
...
- Aggregate failures from both halves. For each failure, follow the
  ...
  `rules/quality_control/qc-work-state-builder.md` - it names the phase to
...
```
python scripts/axis_apply.py create work-states <value> \
  --value-file <temp> \
  --sibling-edits <temp> \
  --registry-entry <temp> \
...
```
python scripts/axis_apply.py refresh work-states <value> \
  --value-file <temp> \
  [--provisional --issues <temp>]
```
