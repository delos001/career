---
name: orientation-builder-reconciler
description: Reconciles an orientation-builder draft against existing rules/orientations/ files. In create mode, receives each sibling's Adjacency-section slice (extracted upstream by scripts/axis_registry.py slice) and drafts per-sibling Adjacency back-edges in each sibling's voice. In refresh mode, reads the current value file and emits a structured change list against the new draft. Returns JSON that scripts/axis_qc.py and scripts/axis_apply.py consume directly.
tools: Read
---

# Orientation builder reconciler

You reconcile a drafted orientation value file against the rest of
`rules/orientations/`. You do not write to disk. You return structured JSON
that the dispatching `orientation-builder` skill passes to
`scripts/axis_qc.py` (for the I3 no-op-refresh check) and
`scripts/axis_apply.py` (for the actual file writes).
...
- `research_findings` (refresh mode only): the research block produced by
  `orientation-builder-research`, so the reasoning for each change can
  cite the source.
...
3. If the section does not cover the new value, judge whether the pair
   ...
   - **Substantive back-edge bullet** when a translation rule exists:
     ...
     - Reads in the sibling's voice: when the sibling is selected for the
       CV, does an entry tagged with the new value translate? Under what
       re-anchoring conditions?
     - Matches the phrasing style of the sibling's existing Adjacency
       bullets in `adjacency_text`. Those bullets are your voice reference.
     - States both the translation case and the non-translation case
       (when does it NOT translate even after reframing).
   - **Plain low-form bullet** when there is no real translation rule
     to state (the pair has no meaningful CV-framing relationship): the
     bullet is exactly `- <value>` on its own line. The apply step
...
2. For each section in the schema (`Identity`, `Summary lead`,
   `Section emphasis`, `Adjacency`), compare the current text against
   the drafted text. The `## Adjacency` section may contain two forms
   per `axes-file-schema`: substantive bullets and plain-name bullets
   inside the mandatory `### Low or no adjacency` sub-section. A change
   from one form to the other for the same sibling is a substantive
   difference; emit it as a `modify` change naming the sibling and the
   form transition.
3. Emit one change record per substantive difference. A substantive
   difference is one that changes meaning (a new exclusion condition,
   a reworded identity statement, a new summary-lead concept, a form
   transition in Adjacency). Pure whitespace, punctuation-only edits,
   and reorderings that do not change meaning are skipped.
...
```json
{
  ...
  "sibling_edits": [
    {
      "sibling_file": "<filename, e.g. transformation-strategy.md>",
      "section": "Adjacency",
