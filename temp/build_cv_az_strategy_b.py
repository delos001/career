"""Build CV for AstraZeneca Director, Process Management — Strategy (b): pure semantic Description ranking, no tag filter."""
...
add_body(doc, (
    ...
    "Lean Six Sigma Black Belt with applied experience in value stream mapping, SIPOC, formal hypothesis "
    "testing, root cause analysis, and KPI design. Targets director-level process management roles where "
    "process ownership, regulatory-grade documentation, governance, and continuous improvement methodology "
    "drive clinical execution quality."
))

add_section_header(doc, 'Core Competencies')
for c in [
    'Clinical process design, documentation, and lifecycle ownership',
    'Lean Six Sigma methodology (VSM, SIPOC, RCA, gap analysis, hypothesis testing)',
    ...
    'Data-driven process decisions and risk stratification',
]:
    add_bullet(doc, c)
...
for b in [
    ...
    'Championed integration of data acquisition workflows into downstream regulatory submission, AI readiness, and quality processes; institutionalized adoption through training and recurring governance forums.',
...
for b in [
    ...
    'Replaced legacy data freezing process with a longitudinal stability surveillance capability tracking daily critical data point changes; change detection moved from effectively zero to 100% and freeze man-hours reduced to zero on the pivotal study.',
...
for b in [
    ...
    'Built Spotfire dashboards delivering near-real-time operational visibility (last patient visit, query volume, data acquisition status) across two pivotal studies; replaced weekly spreadsheets and gap reporting.',
]:
    add_bullet(doc, b)
...
for b in [
    ...
    'Built Time Series ARIMA models predicting query and data entry volumes at critical milestones, providing forward-looking resource visibility and early indicators of deliverable risk.',
]:
    add_bullet(doc, b)
...
for b in [
    ...
    'Led CSM hyper-care meetings to evaluate signal review compliance, collect program feedback, and identify opportunities for continuous improvement of the CSM offering.',
...
for b in [
    ...
    'Applied formal statistical hypothesis testing (two-proportion and mean z-tests) to evaluate sponsor-proposed site closure across 238 global sites; advised that the data did not support specialty-based closure. Sponsor abandoned the closure plan.',
]:
    add_bullet(doc, b)

# Earlier Roles (incl. NeoVista, Duke; pre-2007 Background Roles for completeness)
add_section_header(doc, 'Earlier Professional Roles')
for b in [
    'Clinical Research Associate II | NeoVista Inc. (medical device) | 2011',
    'Clinical Research Associate I/II | The Duke Clinical Research Institute | 2007 to 2011 — pioneered data-driven site risk stratification in 2009, predating FDA RBM guidance.',
...
out = r'C:\Users\delos\code\career\temp\Jason_Delosh_CV_AZ_DirProcessMgmt_2026-05_strategy-b.docx'
doc.save(out)
print(f'Saved: {out}')
