---
qc_target: orientation-builder
last_updated: 2026-05
---

# QC - orientation-builder

**Used by:** qc-orientation-builder, orientation-builder, scripts/axis_qc.py, scripts/axis_apply.py

Quality checks the `qc-orientation-builder` subagent runs against a drafted
orientation value file plus the per-sibling Adjacency edits (create mode) or
...
- **`script`** - `scripts/axis_qc.py` runs the check and, where the fix is
  purely textual, auto-fixes the value file in place.
- **`subagent`** - `qc-orientation-builder` subagent runs the check in
...
- **A1** *(script)*: Frontmatter present and bounded by `---` fences at
  ...
  - Fix on fail: report; Phase 5 routes the failure to Phase 3 redraft.
- **A2** *(script)*: `orientation: <value>` key present and matches the
  value being built.
...
- **A4** *(script)*: A top-level title line is present and ends with
  `- CV Framing Rules` (case-insensitive on the suffix).
...
- **B1** *(script)*: All four required sections present, unique, and
  non-empty: `## Identity`, `## Summary lead`, `## Section emphasis`,
  `## Adjacency`. Duplicate headings fail B1 too.
...
- **C1** *(subagent)*: Every named framework, methodology, or convention
  in the drafted file traces to a source in `research_findings`.
...
- **D1** *(subagent)*: The drafted file's Identity section does not
  paraphrase the identity of a sibling orientation. Each orientation's
...
- **D2** *(subagent)*: The drafted file's content stays within the
  ...
  industry-specific terminology or regulatory framing (industries
...
- **E1** *(script)*: The drafted file's `## Adjacency` section enumerates
  every non-self entry in `rules/orientations/registry.md` in one of two
...
- **E3** *(subagent, create only)*: Each per-sibling back-edge bullet is
  phrased in that sibling's voice (describes when this orientation's
  entries translate to the sibling's CV framing).
...
- **F1** *(subagent)*: Sources cited in the research findings match the
  acceptable source class for the claim type they support. Orientation
  files are framing-heavy and rarely cite hard regulatory or quantitative
  claims; F1 typically applies to hiring-pattern and CV-convention claims.
  Acceptable sources at that tier: an authoritative reference if one
  exists at the claim's granularity, otherwise verified industry
  intelligence (recognized recruiter firms, established trade
  publications, sector-specific newsletters with editorial accountability).
  Marketing pages, unsourced blogs, sponsored content, and AI-generated
  summaries without attribution fail F1.
...
list to the `--issues` temp file passed to `scripts/axis_apply.py`. Each
entry:
