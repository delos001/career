...
- Trigger: user direction; OR next cv-targeted run where L1 forces a content drop on a small overage; OR next cv-targeted run (proactively surface redundancy/over-granularity at handoff, then review specimens all-at-once and write the consolidation rule).
