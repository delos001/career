...
### f1-source-authority-tiering-revisit
F1 (source authority) currently treats all "marketing pages and low-authority blogs" as non-authoritative even when the underlying claim is uncontroversial and no authoritative source exists at the claim's granularity. For hiring-pattern claims (what hiring panels weight in a given industry), authoritative survey data structurally does not exist at industry-role granularity: AAMI / AdvaMed / ACLA / CAP / AMP / CLSI / BLS publish no IVD-manufacturer hiring-panel survey data, and the same structural gap holds for the sibling industries. Verified industry intelligence (recruiter-firm reporting, sector-specific newsletters, established industry-trade publications) is the practical source class for these claims. Surfaced 2026-05-18 during /industry-builder diagnostics test: F1 fired on the Emphasis section because it cited Cerca Talent (recruiter blog) for hiring-pattern claims; scoped re-research confirmed no authoritative survey exists at this granularity. User framing: "a verified recruiter's blog is on the cusp but I wouldn't totally ignore it." F1 should tier source authority by claim type: regulatory / quantitative / temporal claims require authoritative sources (current rule); hiring-pattern / qualitative-industry-sentiment claims should accept verified industry intelligence when authoritative is structurally unavailable. Options to evaluate when triggered: (a) per-claim-type tier table in the F1 spec; (b) a "best-available-given-structural-gap" pass condition; (c) a per-file frontmatter field declaring source-class expectations.
- Trigger: before next /industry-builder run that would exercise F1 on hiring-pattern claims (essentially every create or refresh); or before replicating the builder pattern to specialty / orientation / level / work-state builders (each axis will have similar Emphasis-section hiring-pattern claims).
- Blocks: clean ships on industry create / refresh runs where Emphasis claims need recruiter-intelligence sources.
- Refs: `rules/quality_control/qc-industry-builder.md` (F1 definition), `.claude/agents/qc-industry-builder.md` (F1 check), `design/build_issues.md` (2026-05-18 diagnostics F1 entry), `qc-h2-mechanical-script-check-2026-05` (parallel pattern of moving an over-strict subagent check to mechanical with tiering).

### axis-builder-script-split
`scripts/axis_builder.py` currently bundles four distinct concerns as subcommands of one CLI: registry parsing (`list`), section retrieval (`slice`), mechanical QC (`qc`), and file writes (`apply-create`, `apply-refresh`). Bundling happened incrementally as features were added; shared helpers (`_find_section_bounds`, `_split_frontmatter`, `_read`, `_unwrap_list`, etc.) and a single argparse dispatcher made it easy to keep adding. Per repo rule `scripts-separate-by-concern` (memory feedback captured 2026-05-18), scripts organize by concern: builder, QC, retrieval, scraping. Target split: `scripts/axis_registry.py` (list, slice), `scripts/axis_qc.py` (qc), `scripts/axis_apply.py` (apply-create, apply-refresh), with shared helpers in `scripts/axis_utils.py`. Refactor also requires updating `.claude/skills/industry-builder/SKILL.md` command references throughout (Phase 1 `list` call; Phase 4 `slice` calls; Phase 5 `qc` call; Phase 6 `apply-create` / `apply-refresh` calls). Surfaced 2026-05-18 during /industry-builder diagnostics post-mortem when user pushed back on the smooshed structure.
- Trigger: before next refactor that touches `axis_builder.py`; or before replicating the builder pattern to specialty / orientation / level / work-state (replicating the smooshed pattern would compound the issue).
- Blocks: nothing currently (the bundled CLI works); blocks clean structural rule application across the axis-builder family.
- Refs: `scripts/axis_builder.py`, `.claude/skills/industry-builder/SKILL.md`, `feedback_scripts_separate_by_concern.md`, `builders-axis-parity`.
