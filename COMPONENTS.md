...
- **Purpose**: Deterministic mutations for `inventory.md` and the staging file, for the `profile-update` skill. Subcommands: `pending` (unprocessed staging entries, compact), `show` (one staging entry in full), `next-id` (next unused ID for a prefix), `insert` (place a new entry in sorted position, assigning its ID and rebuilding the table of contents), `set` (replace one field's value on one entry, the enrichment path), `list-add` (append one item to a list line in a section that holds flat lists rather than ID-bearing entries), `close` (flip a staging entry to processed and write its `Processed:` audit line), `drop` (remove a staging entry the user retracts; refuses a processed one). Owns the inventory's schema parser and its target-resolution rule; the QC script imports both rather than re-implementing them.
- **Status**: Designed
- **Inputs**: Subcommand args (`--pu`, `--prefix`, `--block-file`, `--subsection`, `--id`, `--field`, `--value-file`, `--target`, `--item`, repeatable `--targets`, `--date`). Config: `config.yaml` (profile path, templates path, inventory_file, inventory_template, staging_file) via `scripts/_config.py`. Filesystem: `personal/profile/inventory.md`, `personal/profile/profile_updates_pending.md`, `templates/inventory.md` (the section roster and per-prefix field rosters are parsed from it at run time, never hardcoded).
- **Outputs**: Updated `inventory.md` (entry placed, ToC regenerated; or one field replaced; or one list item appended) or updated staging file; assigned ID, list address, or confirmation printed to stdout. Errors to stderr with exit 1. `insert` rejects a block whose fields are missing, unknown, or out of template order, and rejects a block carrying its own `ID:` line (the script assigns IDs so two inserts in one run cannot collide). `list-add` appends only, never rewrites a line, and rejects a duplicate item or one carrying the line's own separator. `close` verifies every target, entry ID or list address, before writing the audit line, and joins them with `; ` because a list address can itself carry a comma.
...
- **Purpose**: Mechanical quality checks for the `profile-update` skill. Eleven checks: section roster and order (P1), per-entry field rosters (P2), ID integrity (P3), EX placement and within-group sort order (P4), `Role` / `Recognizes` reference resolution (P5), axis values against the registries (P6), table-of-contents accuracy (P7), empty-section markers (P8), staging entries closed this run (P9), staging entry integrity (P10), list-section content and per-category duplicate items (P11). Judgment checks belong to `qc-profile-update`.
- **Status**: Designed
